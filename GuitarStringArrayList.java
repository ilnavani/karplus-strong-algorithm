...
import java.util.*;
/**
Implementation of a subclass of GuitarString that creates a ring buffer as an array
...
public class GuitarStringArrayList extends GuitarString {
    ...
    //initializing ring buffer as an ArrayList
    List<Double> myBuffer = new ArrayList<>();
    ...
    public void pluck() {
        for(int i=0; i<myBufferSize; i++) {
            myBuffer.set(i, (Math.random()*(0.5-(-0.5))+(-0.5)));
        }
    }
    /**
    ...
    public void tic() {
        myNumTics++;
        double average = ((myBuffer.get(0) + myBuffer.get(1))/2) * ENERGY_DECAY;
        myBuffer.remove(0);
